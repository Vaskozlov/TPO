...
public class CosStub implements Function {
    @Override
    public double compute(double x, double eps) {
        ...
        if (Math.abs(modX) < 0.01 || Math.abs(modX - 2*Math.PI) < 0.01) return 1.0;
        if (Math.abs(modX - Math.PI/2) < 0.01) return 0.0;
        if (Math.abs(modX - Math.PI) < 0.01) return -1.0;
        if (Math.abs(modX - 3*Math.PI/2) < 0.01) return 0.0;
